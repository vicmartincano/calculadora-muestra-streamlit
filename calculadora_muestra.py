...
st.success(
    f"Debes incluir **al menos {n_final:,}** personas o elementos en la muestra. "
    "El valor se redondea hacia arriba para no perder precisión.".replace(",", ".")
)
...
with st.expander("Ver fórmula y sustitución de valores"):
    ...
    st.latex(
        rf"n = \frac{{{N} \times {z:.3f}^2 \times {p:.2f} \times {q:.2f}}}"
        rf"{{{e:.2f}^2 \times ({N} - 1) + {z:.3f}^2 \times {p:.2f} \times {q:.2f}}}"
        rf" = {n_exact:.2f}"
    )

st.subheader("Ajuste por no respuesta")
st.write(
    "Si esperas que algunas personas no respondan, aumenta el número de invitaciones. "
    "Esto no cambia la muestra válida requerida; solo evita quedarte corto."
)
response_rate = st.slider(
    "Tasa esperada de respuesta (%)",
    min_value=10,
    max_value=100,
    value=80,
    step=5,
...
st.metric(
    "Personas o elementos que conviene invitar/seleccionar",
    f"{invitations:,}".replace(",", "."),
)
